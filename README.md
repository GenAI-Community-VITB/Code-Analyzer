...
- JSON report persisted to disk after each run
...
- **LLM reasoning**: Gemini 2.5 Flash provides natural-language summaries, issues, and suggestions for each file, as well as a concise repository-level design assessment covering structure, layering, cohesion, and design risks.
...
Up to 40 files are sent to Gemini 2.5 Flash in batches of 8 (configurable), with up to 2 concurrent batches. Each file is truncated to 8,000 characters before sending. If a batch fails, the system falls back to per-file requests. LLM analysis is skipped gracefully if `GEMINI_API_KEY` is not set.
...
The full result is serialized to JSON and written to the `reports/` directory. The clone directory is deleted after the response is assembled.
...
| GET | `/analyze?repo_url=<url>` | Analyze a repository |

**Example request**

```bash
curl "http://localhost:8000/analyze?repo_url=https://github.com/owner/repository"
```
...
│       ├── report_generator.py       # JSON report serialization and file persistence
...
- **Shallow clone**: `--depth 1 --single-branch` fetches only the latest commit tree, minimizing download size and time.
- **File cap**: At most 120 files are retained after filtering for static analysis, and at most 40 are forwarded to the LLM.
- **Parallel static analysis**: All files are analyzed concurrently in a thread pool via `asyncio.gather`, avoiding sequential I/O blocking.
- **Batched LLM calls**: Files are grouped into batches of up to 8 and submitted in up to 2 concurrent batches, balancing throughput against API rate limits.
- **Content truncation**: File content sent to the LLM is truncated to 8,000 characters, keeping prompt size bounded.
- **Clone teardown**: The cloned directory is deleted immediately after analysis, keeping disk usage transient.
